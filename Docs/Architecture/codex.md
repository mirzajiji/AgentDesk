...
## Implemented subprocess streaming — P1-09a

`MacProcessRunner` is the internal byte transport now used by the real diagnostics adapter. It accepts argument arrays, an explicit working directory and explicit environment; arbitrary environment variables are not inherited. Optional stdin is fed through a nonblocking pipe while stdout and stderr are drained independently. Nil stdin uses `/dev/null`; empty data closes stdin immediately. Child input closure is handled without delivering SIGPIPE to AgentDesk. UTF-8 decoding and JSON framing belong above this byte boundary so split code points are preserved.

The transport delivers tagged chunks of at most 8 KiB to a synchronous bounded sink before process exit, with a caller-selected combined output budget capped at 16 MiB. Stdin is capped at 1 MiB; argv and environment each have entry and aggregate byte limits. Runtime is capped at one hour; diagnostics keep their existing stricter 10/180-second command deadlines and 64 KiB budget. The sink must return promptly and can throw when its bounded event queue cannot accept data; that error cancels and reaps the producer. This layer performs no persistence or display of raw bytes.

Spawning uses a fresh process group, closed unrelated file descriptors, default catchable signal dispositions and an empty signal mask. Pipe descriptors stay above the standard streams and are close-on-exec. Cancellation, timeout, excess output and sink errors terminate the child group, reap the direct child and close all owned pipes. Descendants retaining stdout/stderr after parent exit are stopped. Exit code and terminating signal remain distinct; neither alone proves task success.

This is a real process capability exercised by synthetic executables and installed CLI diagnostics, not an exposed execution command. `ExecutionProvider`, scoped requests, JSON events, policy and native run coordination still gate agent task execution. See [transport validation](../Development/p1-09a-validation.md).
