...
Refresh and bounded load-more read persisted current versions; stale saves fail rather than replacing a newer version. Disabled and enabled-but-unchecked states are displayed separately from authenticated or healthy status. Saving does not contact Jira. Native sign-in, connection testing, grant lifecycle controls, permission review and image upload review remain unfinished. See [P3-05 validation](../Development/p3-05-validation.md).
