...
P1-11a adds project-scoped keyset pagination for unfinished runs (up to 256 per page) and verified lookup of a run's immutable evidence binding. Recovery verifies the stored environment column against decoded scope/run/environment identity; malformed or inconsistent metadata is an explicit database error. That recovery change needed no migration (schema version 5 at P1-11a). Only the trusted project execution owner uses these reads to recover interrupted runs; wire clients do not gain cross-environment read access. See the [coordinator contract](execution.md).

## Knowledge search cache (schema 6)

P2-06a adds a redacted FTS5 knowledge projection and per-project/environment generations to the operational database. Rebuilds are transactional and preserve other scopes and operational history. Source JSON remains authoritative. Titles/body snapshots are redacted before insertion; search filters and cursors enforce project/environment, classification and logical paths. See [knowledge index](project-memory.md#rebuildable-search-index-p2-06a) and [validation](../Development/p2-06a-validation.md).
