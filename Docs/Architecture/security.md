...
## Explicit image masking primitive

`ImageEvidenceRedactor` processes a supplied single-image buffer into an opaque RGB PNG and applies reviewer-supplied pixel masks measured from the orientation-corrected top-left corner. Re-encoding discards source metadata. Decoding, pixel dimensions, output bytes and mask count are bounded; invalid regions fail rather than being clipped. Cancellation is checked before and after synchronous ImageIO work and between masks, but cannot interrupt an ImageIO call already in progress.

The returned `MaskedImageEvidence` has no Codable conformance, hides bytes in diagnostic representations, and requires the exact redaction context for PNG access. It applies only the supplied masks: zero masks remove metadata only, and unselected pixels remain visible. This is not automatic secret discovery, an assertion that the whole image is sanitized, or permission to export. It is not accepted by the current text evidence store. The PNG upload backend now binds the processed image to explicit write review, a separately authorized settings read and durable attempt tracking. Native image selection/review and image artifact storage still need their own integration. See [validation and remaining integration](../Development/p3-04-validation.md).
