...
The returned `MaskedImageEvidence` has no Codable conformance, hides bytes in diagnostic representations, and requires the exact redaction context for PNG access. It applies only the supplied masks: zero masks remove metadata only, and unselected pixels remain visible. This is not automatic secret discovery, an assertion that the whole image is sanitized, or permission to export. It is not accepted by the current text evidence store. Native review, upload authorization and image artifact storage still need their own integration. See [validation and remaining integration](../Development/p3-04-validation.md).
