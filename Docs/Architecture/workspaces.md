...
The configuration directory's initial trusted-container walk uses search-only directory descriptors (`O_SEARCH`) for ancestors and `O_NOFOLLOW` for each resolved component. This permits traversal into the app's own container without requesting listing access to `/` or parent folders outside its sandbox. Reads and enumeration remain confined to validated child directories. P1-08b adds a search-only ancestor regression and verifies a normal sandboxed app launch; it does not widen filesystem permissions.
