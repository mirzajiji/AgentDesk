...
## Native issue approval lifecycle regression coverage

Introduced a narrow MainActor UI dependency for prepare/review/execute/close. Production wraps `NativeJiraReadReview` without changing authorization; test doubles remain in the native test target. Added model regressions for approval-required no-dispatch, exact approval ID/sequence forwarding, one execution and redacted result display, no repeat approval after completion, pending cancellation without execution, and suppression of an approved response arriving after cancellation.

The initial test used the wrong redactor method name; the corrected `redactText` fixture compiled and the four initial model tests passed. A fifth late-result test and the existing compact-window issue UI test are running in `native-issue-approval-final.xcresult`. This validates UI state transitions with a fake, not live Jira or runtime authorization; the existing Runtime HTTP tests remain the authorization evidence. Full approval-button UI acceptance remains outstanding.
