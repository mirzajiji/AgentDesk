...
## Policy-gated credential environment

Approved MCP startup can now accept a scoped SecretStore. The launch session retains the exact persisted configuration only after verifying its revision fingerprint against the prepared action. During approved startup it performs a separate readSecret policy action before fetching configured references, validates the store's exact workspace/project/environment, checks UTF-8/NUL and total-size limits, and revalidates configuration/policy after reads. Only then are values passed as process environment entries. No values enter the saved configuration or approval payload; no credential logging was added.

The real-process fixture asserts receipt of a synthetic environment value. Its fake store counts reads: allow reads once and starts; deny and approval-required policies read zero times and do not start. All three approved-launch tests pass (`TestResults/p3-06-credential-policy-tests.log`), and all 188 Runtime tests pass (`swift test --package-path Packages/AgentDeskRuntime`, `TestResults/p3-06-credentials-full.log`). The normal Mac build command above passes (`TestResults/p3-06-credentials-mac.log`). macOS 26.5.2 / Xcode 26.0; Mac-only changes, no new iPhone coverage claimed. Documentation and diff checks pass.

Separate readSecret approval preparation/review is still required for policies that demand review: these currently fail closed, even after launch approval. Native presentation must redact all untrusted server output before display, including values a child may echo; the internal handshake description is not a safe UI payload. Native exposure and full MCP acceptance remain unfinished. Counts remain 52 complete, 11 Phase 3 tasks plus Phases 4–6 remaining.

## Separate credential review

MCP launch sessions now retain a stable, distinct readSecret action and expose separate credential preparation/review. Startup can consume its credential approval in addition to process-launch approval. Both actions bind the saved configuration and physical resource; launch approval cannot substitute for credential approval. Existing policy/configuration revalidation applies to both. This resolves the previous internal limitation for review-required secret reads; native review UI remains unfinished.

The synthetic credential test now includes successful startup after both reviews, alongside allow/deny/unreviewed cases, and rejects using a launch approval as the credential approval. All three focused launch tests pass (`TestResults/p3-06-credential-review-tests.log`), all 188 Runtime tests pass (`swift test --package-path Packages/AgentDeskRuntime`, `TestResults/p3-06-credential-review-full.log`), and the normal Mac build command above passes (`TestResults/p3-06-credential-review-mac.log`). macOS 26.5.2 / Xcode 26.0. Mac-only change; no new iPhone coverage claimed. Documentation and diff checks pass. No personal credentials or live services used. Counts unchanged: 52 complete; 11 Phase 3 tasks plus Phases 4–6 remain.

## Redacted startup presentation

Approved startup now returns MCPServerPresentation with scoped RedactedText name/version fields and protocol/capability flags, rather than raw handshake identity. The redactor includes the exact credential values supplied to the child, without rereading Keychain. Redaction failure closes the opened connection. Server claims remain untrusted and do not grant permissions. Future tool/resource/log payloads still require their own redaction before display or persistence.

The synthetic credential fixture echoes its environment credential as its server name; the integration test verifies removal, a positive redaction count and the expected project scope for both allowed and reviewed credential delivery. All three focused launch tests pass (`TestResults/p3-06-identity-redaction-tests.log`); all 188 Runtime tests pass (`swift test --package-path Packages/AgentDeskRuntime`, `TestResults/p3-06-identity-redaction-full.log`); normal Mac build passes (`TestResults/p3-06-identity-redaction-mac.log`) using the command above. macOS 26.5.2 / Xcode 26.0. Mac-only change; no new iPhone coverage claimed. Documentation and diff checks pass. Native MCP UI and remaining transport/discovery acceptance remain unfinished. Counts: 52 complete; 11 Phase 3 tasks plus Phases 4–6 remain.

## Registered repository directory base

MCP configuration now explicitly selects workspace-relative or registered-repository-relative resolution. An omitted base in existing configuration decodes as workspace-relative. Only repository mode permits an absent subdirectory, meaning the registered repository root. Repository mode accepts host-authenticated project roots outside workspace storage, while canonical child containment still rejects symlink escapes. Workspace mode retains its prior containment checks. The native editor exposes the base and preserves root selection across save/reopen. Root URLs still must be supplied by the trusted registration service before native startup is exposed.

Validation: 14 MCP host tests pass (`TestResults/p3-06-directory-base-wire.log`), including legacy decoding and repository-root round trip. Three resource tests pass, including external repository roots and symlink escape rejection (`TestResults/p3-06-directory-root-tests.log`). The initial test compared Foundation's /var spelling with realpath's /private/var spelling; corrected expected canonicalization passes. All 189 Runtime tests pass (`swift test --package-path Packages/AgentDeskRuntime`, `TestResults/p3-06-directory-base-runtime.log`). All 14 MCP tests pass on iPhone 16 Pro / iOS 26.0 using the package command above with result bundle `TestResults/p3-06/directory-base-iphone.xcresult` (`TestResults/p3-06-directory-base-iphone.log`). Native model/UI tests pass using the P3-08 focused command with result bundle `TestResults/p3-08-directory-base.xcresult` (`TestResults/p3-08-directory-base.log`), including saving and reopening empty repository-root selection. Both normal app builds above pass (`TestResults/p3-06-directory-base-mac-build.log`, `TestResults/p3-06-directory-base-iphone-build.log`). macOS 26.5.2 / Xcode 26.0. Documentation and diff checks pass. Counts unchanged: 52 complete; 11 Phase 3 tasks plus Phases 4–6 remain.

## Registered repository grant lifetime

MCP launch sessions now accept a scoped `RepositoryAccess` and retain its security-scoped folder grant and registration lease until `close()` finishes cancelling startup and closing the connection. The registered directory supplies the process root; a foreign project registration is rejected. This is internal runtime integration, not completed native Start/Stop UI or a new sandbox entitlement.

Validation on macOS 26.5.2 / Xcode 26.0:

- `swift test --package-path Packages/AgentDeskRuntime`: 190 tests passed, zero failures (`TestResults/p3-06-repository-lifetime.log`). The new synthetic repository regression drops caller ownership, verifies removal is blocked while the session owns access, and checks balanced grant release and removal after repeated close. Existing process tests cover startup cancellation and connection shutdown.
