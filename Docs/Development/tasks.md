...
| P1-14b | Basic menu bar status and run navigation | Authoritative active/approval/failure state, scoped routing to existing sessions, lifecycle cleanup and native UI tests |
| P1-14c | First-run checks, Settings integration and native layout audit | Recoverable readiness checks, settings routing, fixed-size editor/window audit against compact/large Mac requirements, unit/native UI tests |
...
P1-15 regression follow-up: investigate the intermittent approval/start failure observed by the large-window diff UI test during P1-14a. It passed on focused rerun; the original failed full-suite run remains recorded in [P1-14a validation](p1-14a-validation.md).
