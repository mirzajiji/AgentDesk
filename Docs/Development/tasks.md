...
| P1-06a | Complete: project agent creation/editing, templates, immutable instruction versions, archive/restore and native Mac editor | 43 package tests; 68 native Mac unit + 8 UI and 65 iPhone 16 Pro unit executions pass. See [agent editing validation](p1-06a-validation.md). |
| P1-06b | Layered configuration/instruction composition, scoped skill references, environment constraints and output schema | Effective source order, provenance, missing/invalid references, cycles, output validation and cross-scope denial |
