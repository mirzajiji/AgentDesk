...
/// Untrusted template metadata. Templates stay opaque until a separate RFC 6570 expansion boundary.
/// Decoding does not validate template grammar, construct a URI or authorize a resource read.
public struct MCPResourceTemplateDescription: Decodable, Equatable, Sendable {
...
public struct MCPResourceTemplatePage: Sendable {
    public let scope: ProjectScope
    public let environmentID: EnvironmentID
    public let connectionID: UUID
    public let resourceTemplates: [MCPResourceTemplateDescription]
    public let nextCursor: String?
    /// Exact unredacted wire evidence, including annotations and extensions.
    public let response: Data
}
public enum MCPResourceTemplateDiscovery {
    ...
    public static func decode(_ message: MCPMessage, mode: MCPProtocolMode, scope: ProjectScope,
                              environmentID: EnvironmentID, connectionID: UUID) throws -> MCPResourceTemplatePage {
        ...
        do {
            ...
            guard result.resourceTemplates.count <= 1000, (result.nextCursor?.utf8.count ?? 0) <= 4096 else { throw MCPDiscoveryError.sizeLimit }
            if mode == .modern {
            ...
            var uris = Set<String>()
            for resource in result.resourceTemplates {
                try Task.checkCancellation()
                guard !resource.uriTemplate.isEmpty, resource.uriTemplate.utf8.count <= 4096,
                      !resource.uriTemplate.unicodeScalars.contains(where: { CharacterSet.whitespacesAndNewlines.contains($0) || CharacterSet.controlCharacters.contains($0) }),
                      uris.insert(resource.uriTemplate).inserted else { throw MCPDiscoveryError.invalidResponse }
            ...
            return MCPResourceTemplatePage(scope: scope, environmentID: environmentID, connectionID: connectionID,
                resourceTemplates: result.resourceTemplates, nextCursor: result.nextCursor, response: message.bytes)
    ...
    private struct Result: Decodable {
        ...
        let resourceTemplates: [MCPResourceTemplateDescription]
