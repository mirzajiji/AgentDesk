...
final class SQLiteConnection {
    ...
    private func statement<T>(_ sql: String, _ values: [SQLValue], body: (OpaquePointer) throws -> T) throws -> T {
        ...
        for (index, value) in values.enumerated() {
            ...
            case .document(let text):
                guard !text.utf8.contains(0), text.utf8.count <= 262_144 else { throw OperationalStoreError.invalidInput }
                bound = sqlite3_bind_text(prepared, position, text, -1, Self.transient)
    ...
    static func text(_ statement: OpaquePointer, _ index: Int32, maximumBytes: Int = 65_536) throws -> String {
        ...
        guard (1...262_144).contains(maximumBytes), count <= maximumBytes else { throw OperationalStoreError.invalidDatabase }
...
enum SQLValue {
    case text(String), json(String), document(String), integer(Int64), real(Double)
}
