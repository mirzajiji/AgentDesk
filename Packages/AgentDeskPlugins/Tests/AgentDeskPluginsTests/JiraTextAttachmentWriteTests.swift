...
final class JiraTextAttachmentWriteTests: XCTestCase {
    func testUploadHeadersBodyAndReceiptBinding() throws {
        ...
        let request = try JiraTextAttachmentWrite.make(draft, resource: resource, tokens: tokens, now: Date(timeIntervalSince1970: 1000))
        ...
        XCTAssertThrowsError(try JiraTextAttachmentWrite.make(draft, resource: resource, tokens: tokens, now: tokens.expiresAt))
        let denied = JiraCloudResource(id: resource.id, scopes: [])
        XCTAssertThrowsError(try JiraTextAttachmentWrite.make(draft, resource: denied, tokens: tokens, now: Date(timeIntervalSince1970: 1000)))
        let receipt = try JiraTextAttachmentWrite.decode(.init(status: 200, body: Data(#"[{"id":"123","filename":"evidence.txt","size":3}]"#.utf8)), draft: draft, redactor: redactor)
        XCTAssertEqual(receipt.id, "123")
        for body in ["[]", "{}", #"[{"id":"123","filename":"other.txt","size":3}]"#, #"[{"id":"123","filename":"evidence.txt","size":4}]"#] {
            XCTAssertThrowsError(try JiraTextAttachmentWrite.decode(.init(status: 200, body: Data(body.utf8)), draft: draft, redactor: redactor)) {
                XCTAssertEqual($0 as? JiraMutationError, .outcomeUnknown)
            }
        }
        XCTAssertThrowsError(try JiraTextAttachmentWrite.decode(.init(status: 503, body: Data()), draft: draft, redactor: redactor)) {
            XCTAssertEqual($0 as? JiraMutationError, .outcomeUnknown)
        }
        XCTAssertThrowsError(try JiraTextAttachmentWrite.decode(.init(status: 413, body: Data()), draft: draft, redactor: redactor)) {
