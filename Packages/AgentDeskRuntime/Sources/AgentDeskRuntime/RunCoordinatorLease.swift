...
final class RunCoordinatorLease: Sendable {
    ...
    init(container: URL, scope: ProjectScope) throws {
        guard container.isFileURL else { throw RunCoordinatorError.unsafeStorage }
        ...
        name = ".run-owner-\(scope.workspaceID.rawValue).\(scope.projectID.rawValue).lock"
