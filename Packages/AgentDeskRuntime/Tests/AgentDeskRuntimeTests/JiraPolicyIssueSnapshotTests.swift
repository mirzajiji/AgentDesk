...
@MainActor final class JiraPolicyIssueSnapshotTests: XCTestCase {
    func testSnapshotRequiresReadApprovalAndRejectsReplay() async throws {
        ...
        let vault = try JiraCredentialVault(configuration: config, store: JiraSnapshotSecrets(scope: secretScope))
        try await vault.save(tokens)
        let resource = JiraCloudResource(id: UUID(), scopes: ["read:jira-work"])
        let transport = try JiraHTTPTransport(origin: resource.apiOrigin, protocolClasses: [JiraSnapshotProtocol.self])
        ...
        for disposition: PolicyDisposition in [.deny, .approval] {
            ...
            do {
                _ = try await gate.executeJiraIssueSnapshot(identifier: "A-1", connection: connection, permissions: permissions, context: context, redactor: redactor)
            ...
            XCTAssertEqual(JiraSnapshotProtocol.counts.withLock { $0[path, default: 0] }, 0)
            if disposition == .approval {
                ...
                let result = try await gate.executeJiraIssueSnapshot(identifier: "A-1", connection: connection, permissions: permissions, context: context, redactor: redactor, approvalID: pending.id)
                guard case .executed(let snapshot) = result else { return XCTFail("Snapshot did not execute") }
                let edit = try snapshot.edit(summary: redactor.redactText("After", in: context))
                XCTAssertEqual(edit.expectedIssue, snapshot.fingerprint)
                XCTAssertEqual(edit.identifier, "A-1")
                XCTAssertEqual(snapshot.cloudID, resource.id)
                XCTAssertEqual(JiraSnapshotProtocol.counts.withLock { $0[path, default: 0] }, 1)
                do {
                    _ = try await gate.executeJiraIssueSnapshot(identifier: "A-1", connection: connection, permissions: permissions, context: context, redactor: redactor, approvalID: pending.id)
        ...
        _ = JiraSnapshotProtocol.counts.withLock { $0.removeValue(forKey: path) }
    }
}
private actor JiraSnapshotSecrets: SecretStore {
...
private final class JiraSnapshotProtocol: URLProtocol {
