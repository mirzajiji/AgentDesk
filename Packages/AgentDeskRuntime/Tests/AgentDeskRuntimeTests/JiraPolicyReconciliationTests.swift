...
@MainActor final class JiraPolicyReconciliationTests: XCTestCase {
    func testReconciliationRequiresReadApprovalAndRejectsReplay() async throws {
        ...
        let vault = try JiraCredentialVault(configuration: config, store: JiraReconciliationSecrets(scope: secretScope))
        try await vault.save(tokens)
        let resource = JiraCloudResource(id: UUID(), scopes: ["read:jira-work"])
        let transport = try JiraHTTPTransport(origin: resource.apiOrigin, protocolClasses: [JiraReconciliationProtocol.self])
        ...
        let path = resource.apiOrigin.path + "/rest/api/3/issue/A-1/comment"
        for disposition: PolicyDisposition in [.deny, .approval] {
            let permissions = try PluginPermissions(connectionID: config.id, scope: scope, environmentID: environment, rules: [.init(.commentsRead, disposition)])
            let operation = JiraReadOperation.comments(identifier: "A-1", startAt: 0, limit: 20)
            ...
            do {
                _ = try await gate.reconcileJiraComment(draft, startAt: 0, limit: 20, connection: connection, permissions: permissions, redactor: redactor)
            ...
            XCTAssertEqual(JiraReconciliationProtocol.counts.withLock { $0[path, default: 0] }, 0)
            if disposition == .approval {
                ...
                _ = try await gate.reconcileJiraComment(draft, startAt: 0, limit: 20, connection: connection, permissions: permissions, redactor: redactor, approvalID: pending.id)
                XCTAssertEqual(JiraReconciliationProtocol.counts.withLock { $0[path, default: 0] }, 1)
                do {
                    _ = try await gate.reconcileJiraComment(draft, startAt: 0, limit: 20, connection: connection, permissions: permissions, redactor: redactor, approvalID: pending.id)
        ...
        _ = JiraReconciliationProtocol.counts.withLock { $0.removeValue(forKey: path) }
    }
}
private actor JiraReconciliationSecrets: SecretStore {
...
private final class JiraReconciliationProtocol: URLProtocol {
    ...
    override func startLoading() {
        ...
        client?.urlProtocol(self, didLoad: Data(#"{"startAt":0,"maxResults":20,"total":0,"comments":[]}"#.utf8))
