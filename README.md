...
Project memory storage now separates confirmed knowledge, notes and inbox items, preserves immutable revisions/source provenance and requires review for authoritative changes. The native Memory browser supports search/filtering, reviewed edits and promotion, archive/ignore, and earlier versions. See [native memory validation](Docs/Development/p2-10a-validation.md).
...
The persistent Bug Registry retains reviewed defect versions, manual ticket associations, exact references and scoped relationships. Duplicate-review services compare current behavior and requirements, prepare policy-bound Codex ambiguity reviews, retain explicit local decisions, and draft sanitized evidence for known tickets. The native Bug Registry supports reviewed edits, ticket associations, immutable history and direct/inverse bug links. Traceability and duplicate/report review UI remain upcoming; external tickets are not sent. See [native registry validation](Docs/Development/p2-10b-validation.md). See [registry validation](Docs/Development/p2-07-validation.md) and [duplicate-review validation](Docs/Development/p2-08-validation.md).

The native skill editor includes the versioned CityPay bug-report template. Report services preserve current requirements and evidence, group matching roots, redact secrets and reject unresolved duplicates. See [CityPay validation](Docs/Development/p2-09-validation.md).
