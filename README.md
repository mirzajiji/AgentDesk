...
Project memory storage now separates confirmed knowledge, notes and inbox items, preserves immutable revisions/source provenance and requires review for authoritative changes. Native management and retrieval remain upcoming; see [memory validation](Docs/Development/p2-05-validation.md).

A scoped FTS5 knowledge index now supports redacted search, classification/path filters and rebuild-safe pagination. Run-context integration remains the next task; see [index validation](Docs/Development/p2-06a-validation.md).
