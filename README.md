...
**Status:** Phase 1 is underway on `codex/native-foundation`. The Mac creates, renames and reopens local workspaces and projects. Project agent templates, instruction editing and immutable revisions are implemented, alongside scoped configuration, SQLite run storage, persisted run lifecycle/events, native Keychain support, Codex Settings through a signed Mac helper, and an internal read-only Codex execution adapter. Native validation is recorded per task. Personal GitHub push succeeds. See the [implementation tasks](Docs/Development/tasks.md).
