...
Authorization attempts accept an optional `access` value of `read` (default) or `write`. Write consent adds `write:jira-work` to the existing read/offline scopes. Arbitrary scope strings are rejected. The native client checks the returned authorization URL against its explicit choice. Publisher registration must enable the requested scopes; consent and Jira scope availability never bypass AgentDesk's per-operation review.
